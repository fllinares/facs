...
// of the transaction database. Identical transactions will be merged.
// INPUTS:
...
//			This will be a vertical representation, with identical transactions merged and infrequent items
//			and perfect extensions of the root removed
...
int read_transactions(const string filename, Database &db, vector<int> &pexs){
	...
	// Temporal transaction database (in vertical representation), with identical transactions merged,
	Database tmp_db;
...
int depth(Database &db, vector<int> &iset, vector<int> &pexs, vector< vector<bool> *> &elim){
	...
	// Output of recursion
	int r_out;
	// Variable to compute itemset pvalue (only used if compute_pals==true)
	int a;
	double pval;
	...
	for(k=0; k < db_size; ++k){
		...
		// Between the time the item {db.items[k_s]} was added to the current conditional database during the processing
