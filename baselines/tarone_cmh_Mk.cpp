...
#ifndef _tarone_cmh_Mk_cpp_
#define _tarone_cmh_Mk_cpp_
...
double compute_lower_envelope_minpval(vector<int> &x_t){
	...
	for(int c=0; c<=n_cat; ++c) z[c] = 0;
	for(int c=0; c<n_cat; ++c) limits[c] = x_t[c]+1;
	limits[n_cat] = 1;
	int p=0;

	while(z[n_cat]==0){
		for(c=0; c < n_cat; ++c) z_in[c] = z[c];
		minpval = compute_minpval(z_in);
		if(minpval < lower_envelope_minpval) lower_envelope_minpval = minpval;

		z[0]++;
		while((p < n_cat) and (z[p] == limits[p]) ){
			z[p] = 0;
			z[++p]++;
			if(z[p] != limits[p]) p = 0;
		}
