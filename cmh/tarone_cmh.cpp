...
void decrease_threshold(){
	...
	//cout << "idx_th: " << idx_th << ", testability threshold: " << pth << ", n_testable: " << n_testable << endl;
...
/* Given the margins of the n_cat tables and the minimum attainable CMH p-value, checks whether the itemset
 * and all other itemsets containing it can be pruned from the search space
 * */
double compute_lower_envelope_minpval(vector<int> &x_t){
	...
	for(c=0; c<n_cat; ++c) if(x_t[c] > hypercorner_bnd[c]) return -1; // pth>0, hence -1<=pth evalues to true always (itemset never prunable)

	//tic2 = measureTime();

	// Compute the maximum value of left handside function
	for(c=0, cc=0; c<n_cat; ++c){
		// Discard all dimensions for which x_t[c]==0, as they don't contribute to the function neither in the
		// numerator nor the denominator
		if(x_t[c] > 0){
			f_vals[cc] = gamma_t[c]*x_t[c];
			g_vals[cc] = gammabin_t[c]*x_t[c]*(1-((double)x_t[c])/n_samples_t[c]);
			betas[cc] = g_vals[cc]/f_vals[cc];
			idx_betas_sorted[cc] = cc;
			cc++;
		}
	}
	// Sort idx_betas according to increasing order of betas
	sort(idx_betas_sorted.begin(), idx_betas_sorted.begin()+cc, [&](size_t i, size_t j){ return betas[i] < betas[j]; });
	f_sum = 0; g_sum = 0; Tcmh_max_corner_l = 0;
	for(c=0; c<cc; ++c){
		f_sum += f_vals[idx_betas_sorted[c]];
		g_sum += g_vals[idx_betas_sorted[c]];
	...
	// Sort idx_betas according to increasing order of betas
	partial_sort(idx_betas_sorted.begin(), idx_betas_sorted.begin()+cc, idx_betas_sorted.end(), [&](size_t i, size_t j){ return betas[i] < betas[j]; });
